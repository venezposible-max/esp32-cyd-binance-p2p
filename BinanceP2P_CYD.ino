...
const unsigned long FETCH_INTERVAL_MS = 10000; // Cada 10 segundos
...
const unsigned long BCV_FETCH_INTERVAL_MS = 60000; // Consultar BCV cada 60 segundos
String bcvUpdatedAt            = "--:--";
...
void drawWifiIcon(int x, int y, bool connected, int32_t rssi) {
  ...
  tft.fillRoundRect(x, y, 36, 16, 3, boxBg);
  tft.drawRoundRect(x, y, 36, 16, 3, border);

  if (!connected) {
    tft.setTextColor(COLOR_SELL_RED, boxBg);
    tft.drawString("!WF", x + 8, y + 4, 1);
  } else {
    int bars = 1;
    if (rssi >= -65) bars = 4;
...
bool fetchBinanceP2P(String tradeType) {
  ...
  if (httpCode == HTTP_CODE_OK) {
    ...
    if (!error && doc["success"]) {
      lastUpdatedStr = doc["updatedAt"].as<String>();
      JsonArray arr = doc["ads"].as<JsonArray>();
      
      adsCount = 0;
      for (JsonObject item : arr) {
        if (adsCount >= MAX_ADS) break;
        adsList[adsCount].name     = item["name"].as<String>();
        adsList[adsCount].orders   = item["orders"].as<int>();
        adsList[adsCount].rate     = item["rate"].as<String>();
        adsList[adsCount].price    = item["price"].as<String>();
        adsList[adsCount].minLimit = item["min"].as<String>();
        adsList[adsCount].maxLimit = item["max"].as<String>();
        adsList[adsCount].crypto   = item["crypto"].as<String>();
  ...
  for (;;) {
    ...
    if (requestImmediateFetch) {
      ...
      if (!isKeypadOpen && !isBankModalOpen && !isWifiListOpen && !isWifiKbOpen) {
        if (now - lastFetchMillis >= FETCH_INTERVAL_MS) {
          shouldFetchBinance = true;
        }
...
void drawHeader() {
  ...
  // Insignia Logo CREALO
  tft.fillRoundRect(8, 4, 16, 16, 4, COLOR_CYAN);
  tft.setTextColor(COLOR_HEADER_BG, COLOR_CYAN);
  tft.drawCentreString("C", 16, 5, 2);
  
  // Marca: CREALO p2p
  tft.setTextColor(COLOR_TEXT_WHITE, COLOR_HEADER_BG);
  tft.drawString("CREALO", 28, 5, 2);
  int cW = tft.textWidth("CREALO", 2);
  tft.setTextColor(COLOR_CYAN, COLOR_HEADER_BG);
  tft.drawString(" p2p", 28 + cW, 5, 2);

  // Icono WiFi interactivo en el centro (x=142)
  bool conn = (WiFi.status() == WL_CONNECTED);
  int32_t sig = conn ? WiFi.RSSI() : -100;
  drawWifiIcon(142, 4, conn, sig);

  // Botón interactivo CALC (x=196..238, y=3..21)
  tft.fillRoundRect(196, 3, 44, 18, 4, tft.color565(0x28, 0x36, 0x4A));
  tft.drawRoundRect(196, 3, 44, 18, 4, COLOR_BINANCE_YEL);
  tft.setTextColor(COLOR_BINANCE_YEL, tft.color565(0x28, 0x36, 0x4A));
  tft.drawCentreString("CALC", 218, 5, 2);

  // Hora compacta y limpia a la derecha (espacio libre garantizado)
  tft.setTextColor(COLOR_TEXT_GRAY, COLOR_HEADER_BG);
  tft.drawRightString(formatCompactTime(lastUpdatedStr), 312, 5, 2);
}
...
void drawAdsList() {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    char initial = adsList[adIdx].name.length() > 0 ? adsList[adIdx].name[0] : 'U';
    ...
    String displayName = adsList[adIdx].name;
    ...
    tft.setTextColor(COLOR_TEXT_GRAY, COLOR_CARD_BG);
    String stats = String(adsList[adIdx].orders) + " ord | " + adsList[adIdx].rate;
    ...
    tft.drawString("Bs " + adsList[adIdx].price, 10, cardY + 24, 4);
    ...
    String cryptoStr = adsList[adIdx].crypto;
    ...
    String lim = "Lim: " + adsList[adIdx].minLimit + " - " + adsList[adIdx].maxLimit;
...
void drawCalculatorScreen() {
  if (calcStep == -1) {
    ...
    if (calcInputStr.length() == 0 && adsCount > 0) {
...
void drawBottomBar() {
  ...
  if (bcvRate > 0.0) {
    ...
    String posStr = "Pag " + String(currentCardPage + 1) + "/" + String(totalPages) + " (" + String(adsCount) + ")";
    tft.setTextColor(COLOR_TEXT_GRAY, COLOR_HEADER_BG);
...
void updateScreensaver() {
  ...
  if (bcvRate > 0.0 && adsCount > 0) {
    float binanceVal = adsList[0].price.toFloat();
    if (binanceVal > 0.0) {
...
void handleTouch() {
  ...
  // Toque en el Botón CALC del Encabezado (y <= 24, x >= 190 && x <= 245)
  if (y <= 24 && x >= 190 && x <= 245 && (now - lastTouchMs >= 200)) {
    lastTouchMs = now;
    openCalculator();
    return;
  }

  // Toque en el Icono WiFi del Encabezado (y <= 24, x >= 135 && x <= 185)
  if (y <= 24 && x >= 135 && x <= 185 && (now - lastTouchMs >= 200)) {
    lastTouchMs = now;
    isWifiListOpen = true;
    wifiListPage = 0;
    scanWifiNetworks();
...
void loop() {
  ...
  if (!isScreensaverActive) {
    // Solo si no hay ningún menú, teclado o calculadora abierta
    if (!isKeypadOpen && !isBankModalOpen && !isWifiListOpen && !isWifiKbOpen && !isCalcOpen) {
      if (now - lastUserInteractionMs >= SCREENSAVER_TIMEOUT_MS) {
        enterScreensaver();
      }
    } else {
      // Si hay un teclado, modal o calculadora abierta, mantener activo el temporizador
      lastUserInteractionMs = now;
    }
  }
